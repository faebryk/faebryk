...
def generate_component(symbol, annotation_properties):
    ...
    # interfaces
    raw_pins = {no: pin for symbol_2 in symbol.get("symbols", {}).values() for no,pin in symbol_2.get("pins", {}).items()}
    ...
    ifs_exp = "\n        ".join([
        ...
        for pinno,_if in faebryk_if_map.items() if pins[pinno]["name"] != "~"
    ])
    ...
    if footprint_str != "":
        ...
        pinmap_trait = "has_defined_footprint_pinmap({})".format("{"+", ".join([
            f"{pinno}: self.IFs.{_if}" for pinno,_if in faebryk_if_map.items()
        ])+"}")
        traits.append(pinmap_trait)

    #
    traits.append("has_defined_kicad_ref(\"{}\")".format(symbol["properties"]["Reference"]))

    
    traits_exp = "\n        ".join([
        f"self.add_trait({trait})" for trait in traits
    ])
